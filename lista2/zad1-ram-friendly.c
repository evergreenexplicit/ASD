...
void mQuickSort(int* keys,int number){
	...
	while(1){
		...
		mQuickSort(&(keys[j+1]),number-(j+1));
...
int main(int argc, char **argv){
	...
	if(!strcmp(argv[2],"--type")){
		...
		if(!strcmp(argv[3],"select"))
			sortMeasurement(selectSort,keys,number,
					&(tmpSwapNum),
					&(tmpCompNum),
					&(execTime));
		else if(!strcmp(argv[3],"insert"))
			sortMeasurement(insertSort,keys,number,
					&(tmpSwapNum),
					&(tmpCompNum),
					&(execTime));
		else if(!strcmp(argv[3],"heap"))
			sortMeasurement(heapSort,keys,number,
					&(tmpSwapNum),
					&(tmpCompNum),
					&(execTime));
		else if(!strcmp(argv[3],"quick"))
			sortMeasurement(quickSort,keys,number,
					&(tmpSwapNum),
					&(tmpCompNum),
					&(execTime));
		else if(!strcmp(argv[3],"mQuick"))
			sortMeasurement(mQuickSort,keys,number,
					&(tmpSwapNum),
					&(tmpCompNum),
					&(execTime));
		
		printf("Number provided: %d\n",number);
		printf("number of comparisons: %d\nnumber of swaps:%d\nexecTime:%lf\n",tmpCompNum,tmpSwapNum,execTime);
		show(keys,number);
		...
		for(int n = 100;n <=10000;n+=100){
			...
			for(int k = 0; k < repCount;k++){
				...
				fprintf(f, "select;%d;%d;%d;%lf\n", n,selectSwapNum,selectCompNum,selectTime); 
				fprintf(f, "insert;%d;%d;%d;%lf\n", n,insertSwapNum,insertCompNum,insertTime); 
				fprintf(f, "heap;%d;%d;%d;%lf\n", n,heapSwapNum,heapCompNum,heapTime); 
				fprintf(f, "quick;%d;%d;%d;%lf\n", n,quickSwapNum,quickCompNum,quickTime); 
				fprintf(f, "mQuick;%d;%d;%d;%lf\n", n,mQuickSwapNum,mQuickCompNum,mQuickTime);
			}			
		}
